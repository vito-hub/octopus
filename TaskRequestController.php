...
private function handleTaskDetail($taskId, $data, $createdTask)
    {
        ...
        if (empty($createdTask)){
            throw new \Exception('task isn`t created properly');
        }
        if (!empty($createdTask->ta_routine_period)) {
            $startDate = Carbon::parse($data['tade_start']);
            $endDate = Carbon::parse($data['tade_end']);
